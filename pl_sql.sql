...
서버에서 일정 시간이 되면 동작하면 프로그램(코드)
스케줄러에 동작하는 job (pl-sql)

back 단 작업 ( 결제 , 승인 .... ) >> 최종 데이터 분석용 ...업무 보고 , 검증 (12 ~ 

pro c >> pl-sql 연동 

출결관리 ....
아침 6시간되면 .. 자동 insert 쿼리 .... 여러분의 출결 사항 null 세팅
타임아웃 버튼 클릭 : 시간을  update 
*/
--PL-SQL
...
--                              v_row.empno , v_row.ename  , v_row.sal , v_row.comm
...
BEGIN
    ...
    IF(vdeptno = 10) THEN vname := 'ACC'; -- if(vdeptno==10) { vname = "ACC"}
        ELSIF(vdeptno=20) THEN vname := 'IT';   -- if    ... else if
...
--2.  여러건의 데이터를 처리하는 방법을 제공 (한 건이상의  row가지고 놀기)
...
--위 표현을 좀 더 간단하게
--List<Emp> emplist 
--java (for(emp e : emplist){}  개선된 for문
    --open 하고 > 실행 > FETCH > row 단위로 emp_record변수 할당
    --emp_record >> empno , ename >> 받아주는 타입 >> INTO vempno , vename, vsal;
 ...
 DECLARE
    ...
    BEGIN
          ...
          IF SQL%NOTFOUND THEN
                 ...
                 --java    public void run() throws IO.... Index....
          END IF;
       EXCEPTION 
        WHEN OTHERS THEN
            ROLLBACK;
...
--Ms-sql : procedure  c#  (99.9)

--create procedure aaa  단독 실행 가능
--create function bbb   select  절에서 사용 
...
(
    param_deptno IN number,
    param_cursor OUT SYS_REFCURSOR  -- JAVA 언어 사용하는 타입 (cursor 다중 데이터)
)
is
    begin
        open  param_cursor
        for select empno ,ename , job, sal from emp where deptno=param_deptno;
    end;
